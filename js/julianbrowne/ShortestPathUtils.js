...
var ShortestPathUtils = {
	...
	makeTable: function(nodes) {
			...
			for(var td_i=0; td_i < (nodes + 1); td_i++) {
				...
				if (tr_i===0) {
					...
					else
					{
						...
						if (td_i > greyCol) td.appendChild(ShortestPathUtils.makeTextInput(td_i-1, tr_i-1));
		...
		var target = ShortestPathUtils.clearDiv(ShortestPathUtils.targetDivForTable);
	...
	addElementProperty: function(element,newProperty,newValue) {
		...
		else if (document.getElementById) element.setAttribute(newProperty, newValue);
