...
/* global $, d3, ShortestPathCalculator */
/* global Town, svgShip, myship, Ship */

// A few globals
var seaLevel = 0.5;
var towns = [];
var mapRender, ShortPathCalc, svgShip, paths, nodes;
...
function addNaviLayer(target, render) {
	...
	view.selectAll('path.field').on("click", function(d, clickedIndex) {
		console.log(myship.node, nodes[myship.node]);		// from
		console.log(clickedIndex, d);				// to
		console.log(nodes[clickedIndex]);
		svgShip.raise();
		myship.findRoute(clickedIndex);
	});
}
...
function addShipSvg(target) {
	...
	myship = new Ship();	// eslint-disable-line no-global-assign
	console.log(myship);
	myship.node = initNode;
}
...
function linkCities(svg) {
	...
	.on('click', function() {
		...
		myship.findRoute(seaNode, function() {
